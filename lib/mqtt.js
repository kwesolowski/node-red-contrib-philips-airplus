...
function createMqttClient({
    ...
    function handleConnect() {
        ...
        log(`[mqtt] handleConnect: ${subscribedDevices.size} devices to resubscribe`);

        // Resubscribe to all devices
        for (const deviceId of subscribedDevices) {
            log(`[mqtt] Subscribing to device ${deviceId}`);
    ...
    async function connect() {
        ...
        log(`[mqtt] Got MQTT info: endpoint=${currentMqttInfo.endpoint}, client_id=${currentMqttInfo.client_id}`);
    ...
    function getDeviceState(deviceId, timeoutMs = 10000) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Publish empty message to shadow/get to request state
