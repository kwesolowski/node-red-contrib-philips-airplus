...
function generateSignature(appId, timestamp, username) {
  ...
  const hmac1 = crypto.createHmac('sha256', MXCHIP_HMAC_SECRET).update(params).digest('hex');

  // Second HMAC: hmac1 with username as key
  const signature = crypto.createHmac('sha256', username).update(hmac1).digest('hex');
...
function createApiClient({ fetchFn = fetch, log = console.log } = {}) {
  ...
  async function getToken(userId) {
    ...
    try {
      const payload = JSON.parse(Buffer.from(mxchipToken.split('.')[1], 'base64url').toString());
