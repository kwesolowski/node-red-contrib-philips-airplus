...
/**
 * Check if an error indicates the refresh token was revoked.
 * This happens when the user changes their password, or after long inactivity.
 * The openid-client library throws an OPError with error='invalid_grant'.
 * @param {Error} err - Error from token refresh
 * @returns {boolean}
 */
function isTokenRevoked(err) {
  return err != null && err.error === 'invalid_grant';
}
